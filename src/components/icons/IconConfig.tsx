...
export function IconConfig({ color, size = 20 }: { color: string; size?: number }) {
  return (
    <View style={[styles.wrap, { width: size, height: size }]}>
      <View style={[styles.outer, { borderColor: color }]}>
        <View style={[styles.inner, { borderColor: color }]} />
      </View>
      <View style={[styles.tooth, styles.top,    { backgroundColor: color }]} />
      <View style={[styles.tooth, styles.bottom, { backgroundColor: color }]} />
      <View style={[styles.tooth, styles.left,   { backgroundColor: color }]} />
      <View style={[styles.tooth, styles.right,  { backgroundColor: color }]} />
    </View>
  );
}
